...
do
    ...
    if [ $SHOULD_UPDATE == "0" ]; then
        ...
        REBUILD_NEEDED=1
        echo "[+] Newer tag for $image found --> $REMOTE_TAG (current: $TAG). Pulling new image."
        ...
        docker stop $(docker ps -a -q --filter="name=$image:$TAG" --format="{{.ID}}")  >> $LOGFILE 2>&1
