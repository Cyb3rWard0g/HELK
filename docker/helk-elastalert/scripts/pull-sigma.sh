...
#                    tools/sigmac -t elastalert -c tools/config/generic/sysmon.yml -c sigmac-config.yml --backend-option keyword_blacklist="*" -o $ESALERT_HOME/rules/sigma_$(basename $rule) $rule
#                    tools/sigmac -t elastalert -c tools/config/generic/windows-audit.yml -c sigmac-config.yml --backend-option keyword_blacklist="*" -o $ESALERT_HOME/rules/sigma_$(basename $rule) $rule
        ...
        for rule in $rule_category/* ; do
            if SIGMAremoveNearRules "$rule"; then
                ...
                tools/sigmac -t elastalert -c sigmac-config.yml --backend-option keyword_blacklist="*" -o $ESALERT_HOME/rules/sigma_$(basename $rule) $rule
                rule_counter=$[$rule_counter +1]
            fi
        done
    fi
