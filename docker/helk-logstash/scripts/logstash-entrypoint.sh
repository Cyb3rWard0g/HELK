...
# Load things from settings folder
settings_folder="./elasticsearch_settings/"
settings_cluster="${settings_folder}settings_cluster.json"
ilm_policy_logs_all="${settings_folder}ilm_policy_logs_all.json"
ilm_policy_elastic_monitoring_all="${settings_folder}ilm_policy_elastic_monitoring_all.json"
index_priority_kibana="${settings_folder}index_priority_kibana.json"
...
for file in "${DIR}"/*.json; do
  while true
    do  
      ...
      ES_STATUS_CODE=$(curl -s -o "/tmp/template_upload_${template_name}" -w '%{http_code}' -X POST "${ELASTICSEARCH_ACCESS}/_template/${template_name}" -d@"${file}" -H 'Content-Type: application/json')
      if [[ "$ES_STATUS_CODE" -eq 200 ]]; then
        break
      else
        cat "/tmp/template_upload_${template_name}"
        echo
...
echo -e "${HELK_INFO_TAG} Configuring kibana index priority.."
until [[ "$(curl -s -o /dev/null -w '%{http_code}' -X PUT "${ELASTICSEARCH_ACCESS}/.kiban*/_settings" -H 'Content-Type: application/json' --data-binary @"$index_priority_kibana")" == "200" ]]; do
  echo -e "${HELK_WARNING_TAG} Retrying kibana index priority"
  sleep 2
done

# *********** Index Life Cycle Management (ILM) Policies ***************
echo -e "${HELK_INFO_TAG} Configuring index life cycle management (ILM) policies.."
until [[ "$(curl -s -o /dev/null -w '%{http_code}' -X PUT "${ELASTICSEARCH_ACCESS}/_ilm/policy/ilm_policy_logs_all" -H 'Content-Type: application/json' --data-binary @"$ilm_policy_logs_all")" == "200" ]]; do
