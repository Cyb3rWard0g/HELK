...
def filter(event)
  ...
  if !domain.nil?
    ...
    event.set("meta_#{@domain}_has_non_ascii", domain_has_non_ascii)
    event.set("meta_#{@domain}_has_whitespace", domain_has_whitespace)
    event.set("meta_#{@domain}_is_idn", domain_is_idn)
  end
